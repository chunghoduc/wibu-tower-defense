...
interface Entry { t: number; level: Level; tag: string; msg: string; data?: unknown; }
...
const realConsole = typeof console !== "undefined"
...
function safe(data: unknown): unknown {
  ...
  try { JSON.stringify(data); return data; } catch { return String(data); }
}

function persist(): void {
  try { localStorage.setItem(LS_KEY, JSON.stringify(buffer.slice(-LS_MAX))); } catch { /* private mode / quota */ }
}
...
function flush(): void {
  ...
  const batch = queue;
  queue = [];
  fetch("/__log", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(batch), keepalive: true })
    .catch(() => { sinkAlive = false; });       // no server (static deploy) — stop trying, keep buffering
}
...
function record(level: Level, tag: string, msg: string, data?: unknown): void {
  ...
  queue.push(e);
  if (level === "error") { showOverlay(e); flush(); persist(); }
  else scheduleFlush();
}
...
function showOverlay(e: Entry): void {
  ...
  if (!box) {
    ...
    box.id = "__errlog";
    box.style.cssText = "position:fixed;left:0;right:0;bottom:0;z-index:99999;max-height:42%;overflow:auto;background:rgba(40,8,8,.94);color:#ffd2d2;font:12px/1.4 monospace;padding:8px 12px;white-space:pre-wrap;border-top:2px solid #ff5a5a";
    const bar = document.createElement("div");
    bar.style.cssText = "position:sticky;top:0;text-align:right;margin-bottom:4px";
    const save = document.createElement("button");
    save.textContent = "⤓ save log";
    save.style.cssText = "background:#2a4a6a;color:#fff;border:1px solid #6a8aba;cursor:pointer;padding:2px 8px;margin-right:6px";
    save.onclick = () => saveLog();
    const close = document.createElement("button");
    close.textContent = "✕ dismiss";
    close.style.cssText = "background:#5a1a1a;color:#fff;border:1px solid #ff5a5a;cursor:pointer;padding:2px 8px";
...
export function installLogger(): void {
  const g = globalThis as unknown as { __log: Entry[]; __saveLog: () => void; __clearLog: () => void };
  g.__log = buffer;
  g.__saveLog = saveLog;
  g.__clearLog = () => { buffer.length = 0; queue = []; try { localStorage.removeItem(LS_KEY); } catch { /* ignore */ } };

  if (typeof window !== "undefined") {
    window.addEventListener("error", (ev) => record("error", "window", ev.message || "error", ev.error ?? `${ev.filename}:${ev.lineno}:${ev.colno}`));
    window.addEventListener("unhandledrejection", (ev) => record("error", "promise", "unhandled rejection", ev.reason));
    window.addEventListener("pagehide", () => {
      try { if (queue.length && navigator.sendBeacon) navigator.sendBeacon("/__log", JSON.stringify(queue)); } catch { /* ignore */ }
    });
  }

  if (typeof console !== "undefined") {
    console.error = (...a: unknown[]) => { record("error", "console", a.map(String).join(" ")); realConsole.error(...a); };
    console.warn = (...a: unknown[]) => { record("warn", "console", a.map(String).join(" ")); realConsole.warn(...a); };
  }
