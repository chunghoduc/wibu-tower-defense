...
/**
 * Core save API: the original battle/summon/loadout/jewel/shop/quest surface.
 * The addictive-features (meta) methods live in the SaveManager subclass so each
 * file stays focused and under the 500-line limit. `save`/`persist` are protected
 * so the subclass can mutate and flush the same in-memory save.
 */
export class SaveManagerCore {
  protected save: HeroSave;

  constructor(private readonly provider: SaveProvider) {
    const loaded = provider.load();
    this.save = loaded ?? SaveManagerCore.freshWithStarters();
    this.persist();
  }
  ...
  protected static freshWithStarters(): HeroSave {
  ...
  resetProgress(): void {
    ...
    this.save = SaveManagerCore.freshWithStarters();
