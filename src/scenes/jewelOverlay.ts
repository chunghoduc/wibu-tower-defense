...
const RARITY_TINT: Record<Rarity, number> = {
  Common: 0x9e9e9e, Magic: 0x4f8cff, Rare: 0xb066ff, Legendary: 0xffb04a, Unique: 0xff5252,
};
...
export class JewelOverlay {
  ...
  openPicker(nodeId: string): void {
    ...
    if (owned.length === 0) {
      panel.add(this.scene.add.text(cx, cy, "No spare jewels.\nWin battles to find more.", {
    ...
    owned.slice(0, 20).forEach((inst, i) => {
      ...
      panel.add(this.jewelTile(tx, ty, inst.defId, def.name, def.rarity, () => {
        this.mgr.socketJewel(nodeId, inst.id);
        this.onChange();
        this.close();
      }, () => this.confirmDestroy(inst.id, def.name)));
    });
    panel.add(this.scene.add.text(cx, cy + 150, "Click a jewel to socket it.  Click ✕ to destroy it forever.", {
  ...
  confirmDestroy(jewelInstanceId: string, name: string): void {
    ...
    panel.add(this.scene.add.text(cx, cy - 40,
      `Destroy "${name}" forever?\nThis cannot be undone.`, {
        fontSize: "15px", color: "#ffcdd2", align: "center", lineSpacing: 6,
      }).setOrigin(0.5));

    panel.add(this.button(cx - 90, cy + 40, "Destroy", "#922b21", "#c0392b", () => {
  ...
  private frame(title: string, w: number, h: number): { cx: number; cy: number; panel: Phaser.GameObjects.Container } {
    const W = this.scene.scale.width, H = this.scene.scale.height;
    const root = this.scene.add.container(0, 0).setDepth(1000);
    const dim = this.scene.add.rectangle(W / 2, H / 2, W, H, 0x000000, 0.6)
      .setInteractive(); // swallow clicks to the tree
    dim.on("pointerdown", () => { /* click-off does nothing; use Cancel/socket */ });
    const cx = W / 2, cy = H / 2;
    const bg = this.scene.add.rectangle(cx, cy, w, h, 0x14181f, 0.98).setStrokeStyle(2, 0x80d8ff, 0.8);
    const titleText = this.scene.add.text(cx, cy - h / 2 + 18, title, {
      fontSize: "17px", color: "#80d8ff", fontStyle: "bold",
    }).setOrigin(0.5);
    const closeX = this.scene.add.text(cx + w / 2 - 18, cy - h / 2 + 6, "✕", {
      fontSize: "16px", color: "#ff8a80",
  ...
  private jewelTile(
    x: number, y: number, defId: string, name: string, rarity: Rarity,
    onPick: () => void, onDestroy: () => void,
  ...
  private button(x: number, y: number, label: string, bg: string, hover: string, onClick: () => void): Phaser.GameObjects.Text {
    const t = this.scene.add.text(x, y, label, {
      fontSize: "14px", color: "#ffffff", backgroundColor: bg,
