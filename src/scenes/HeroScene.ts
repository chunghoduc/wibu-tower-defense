...
export class HeroScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add
      .text(W / 2, 12, "Hero — Loadout", { fontSize: "23px", color: "#ffd700", fontStyle: "bold" })
      .setOrigin(0.5, 0);
    this.add
    ...
    tabs.forEach((tab, i) => {
      const t = crispText(this, this.invRect.x + i * 96, 72, tab.label, {
        fontSize: "12px",
        color: "#fff",
    ...
    pg.fillStyle(0x1a2740, 0.45).fillEllipse(
      dp.x + dp.w / 2,
      dp.y + dp.h * 0.46,
      dp.w * 0.74,
      dp.h * 0.66,
    ...
    if (this.textures.exists(DOLL_BASE_KEY)) {
      const img = this.add
        .image(dp.x + dp.w / 2, dp.y + dp.h / 2, DOLL_BASE_KEY)
        .setOrigin(0.5)
        .setDepth(1);
    ...
    for (const ds of DOLL_SLOTS) {
      ...
      g.lineStyle(1.5, 0x3a4a64, 1).strokeRoundedRect(
        sx - DSIZE / 2,
        sy - DSIZE / 2,
        DSIZE,
        DSIZE,
      ...
      this.add
        .text(sx, sy + DSIZE / 2 + 1, ds.label, { fontSize: "8px", color: "#8aa0bb" })
        .setOrigin(0.5, 0)
        .setDepth(5);
      const zone = this.add
        .zone(sx - 24, sy - 24, 48, 48)
        .setOrigin(0)
        .setRectangleDropZone(48, 48);
    ...
    invG
      .fillStyle(0x0e141e, 0.6)
      .fillRoundedRect(this.invRect.x, this.invRect.y, this.invRect.w, this.invRect.h, 8);
    invG
      .lineStyle(1, 0x2a3650, 1)
      .strokeRoundedRect(this.invRect.x, this.invRect.y, this.invRect.w, this.invRect.h, 8);
    const invZone = this.add
      .zone(this.invRect.x, this.invRect.y, this.invRect.w, this.invRect.h)
      .setOrigin(0)
      .setRectangleDropZone(this.invRect.w, this.invRect.h);
  ...
  private handleDrop(tile: Phaser.GameObjects.Container, zone: Phaser.GameObjects.Zone): void {
    ...
    if (zone.getData("inv")) {
      ...
      if (def && equipSlotsFor(def.slot).includes(slot)) {
        if (!this.mgr.equipItem(instId, slot))
  ...
  private refresh(): void {
    ...
    drawScrollbar(this, this.tiles, {
      x: this.invRect.x + this.invRect.w - 10,
      y: this.invRect.y + 8,
      h: this.invRect.h - 16,
      total: totalRows,
      visible: this.invVisibleRows,
  ...
  private listCount(save: ReturnType<SaveManager["getSave"]>): number {
    ...
    return MATERIALS.filter((m) => kinds.includes(m.kind) && (save.materials[m.id] ?? 0) > 0)
      .length;
  }
  ...
  private gridPos(i: number): { x: number; y: number } | null {
    const c = i % this.invCols,
      r = Math.floor(i / this.invCols) - this.invOffset;
    ...
    if (i === 0) {
      this.tiles.add(
        this.add.text(
          ...
          this.invRect.y + 16,
          isBox
            ? "No chests yet — beat a stage boss to earn one."
            : "No materials yet — clear stages to find them.",
  ...
  private openEnhance(instanceId: string, equipSlot?: ItemSlot): void {
    this.hideTooltip();
    const def = equipSlot
      ? ITEM_CATALOG_MAP.get(
          this.mgr.getSave().inventory.items.find((it) => it.id === instanceId)?.defId ?? "",
        )
      : undefined;
