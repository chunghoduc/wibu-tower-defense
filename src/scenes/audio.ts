...
export function setAudioVolume(v: number): void {
  audioConfig.volume = Math.max(0, Math.min(1, v));
}
...
export class Sfx {
  ...
  attack(ranged: boolean): void {
    if (this.gate("atk", 55))
      this.tone(ranged ? 620 : 340, 0.08, "square", 0.1, ranged ? 880 : 220);
  }
  hit(): void {
    if (this.gate("hit", 50)) this.noise(0.05, 0.1, 1500);
  }
  death(): void {
    if (this.gate("death", 40)) this.tone(300, 0.22, "sawtooth", 0.14, 90);
  }
  cast(): void {
    if (this.gate("cast", 70)) {
      this.tone(440, 0.3, "triangle", 0.16, 1320);
      this.tone(660, 0.3, "sine", 0.1, 1760);
    }
  }
  coin(): void {
    if (this.gate("coin", 60)) this.tone(1180, 0.1, "square", 0.08, 1560);
  }
  enemyHit(): void {
    if (this.gate("ehit", 70)) this.noise(0.06, 0.08, 700);
  }
  place(): void {
    this.tone(520, 0.12, "triangle", 0.18, 760);
  }
  click(): void {
  ...
  win(): void {
    [523, 659, 784, 1046].forEach((f, i) =>
      setTimeout(() => this.tone(f, 0.25, "triangle", 0.2), i * 110),
    );
  }
  lose(): void {
    [392, 311, 233].forEach((f, i) =>
      setTimeout(() => this.tone(f, 0.35, "sawtooth", 0.2), i * 150),
    );
  }
}
