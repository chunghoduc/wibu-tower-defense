...
const REGION_GLYPH: Record<string, GlyphFn> = {
  ...
  arcane: (g, x, y, s, col) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      g.fillStyle(col, 1).fillTriangle(
        x,
        y,
        x + Math.cos(a) * s * 0.45,
        y + Math.sin(a) * s * 0.45,
        x + Math.cos(a + Math.PI / 4) * s * 0.22,
        y + Math.sin(a + Math.PI / 4) * s * 0.22,
      );
    }
  ...
  warden: (g, x, y, s, col) => {
    ...
    g.fillPoints(
      [
        new Phaser.Geom.Point(x - s * 0.65, y - s),
        new Phaser.Geom.Point(x + s * 0.65, y - s),
        new Phaser.Geom.Point(x + s * 0.65, y + s * 0.25),
  ...
  tactician: (g, x, y, s, col) => {
    // coin circle with cross
...
export function drawNodeIcon(
  ...
  if (node.type === "jewel-socket") {
    // already has distinctive shape, skip extra glyph
