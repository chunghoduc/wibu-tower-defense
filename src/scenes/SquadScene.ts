...
import { getTowerStars, getTowerCopies } from "../core/collection.ts";
import { TOWERS } from "../data/towers.ts";
import { ACTIVE_SKILLS_MAP } from "../data/skills.ts";
import { TOWER_ROLES, type Rarity, type TowerRole } from "../data/schema.ts";
...
export class SquadScene extends Phaser.Scene {
  ...
  private drawSlots(): void {
    for (let i = 0; i < SQUAD_MAX; i++) {
      ...
      if (def) {
        this.slotLayer.add(
          makeSlotTile(this, def, i, x, y, w, h, (id) => {
            if (!this.didDrag) this.select(id);
          }),
        );
      } else {
        this.slotLayer.add(
