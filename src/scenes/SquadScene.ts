...
export class SquadScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    crispText(this, 24, 34, "Squad — click to inspect · drag to add/swap:", {
      fontSize: "11px",
      color: "#90a4bb",
    });
    ...
    roles.forEach((role, i) => {
      const t = crispText(this, 74 + i * 64, FY, role ? ROLE_LABEL[role] : "All", {
        fontSize: "11px",
        color: "#fff",
    ...
    const sortBtn = crispText(this, PANEL_X - 8, FY, "", {
      fontSize: "11px",
      color: "#fff",
    ...
    sortBtn.on("pointerup", () => {
      if (!this.didDrag) {
        this.sortKey = this.sortKey === "rarity" ? "name" : "rarity";
        this.redraw();
      }
    });
  ...
  private flashMsg(msg: string, ok: boolean): void {
    ...
    this._feedback = this.add
      .text(this.scale.width / 2, 508, msg, {
        fontSize: "13px",
        color: ok ? "#a5d6a7" : "#ff9b9b",
  ...
  private redraw(): void {
    ...
    for (const t of this.filterTabs) {
      ...
      if (t.getData("sort")) {
        t.setText(`Sort: ${this.sortKey === "rarity" ? "Rarity" : "Name"}`);
        continue;
      }
  ...
  private drawSlots(): void {
    for (let i = 0; i < SQUAD_MAX; i++) {
      ...
      g.lineStyle(
        selected ? 3 : def ? 2.5 : 1.5,
        selected ? 0x7ec8ff : def ? 0xffd24a : 0x3a4a64,
        1,
      ).strokeRoundedRect(x, y, w, h, 6);
      this.slotLayer.add(g);
      this.slotLayer.add(
        crispText(this, x + 5, y + 2, `${i + 1}`, { fontSize: "9px", color: "#6c7a90" }),
      );
      if (def) {
        this.slotLayer.add(this.makeSlotTile(def, i, x, y, w, h));
      } else {
        this.slotLayer.add(
          crispText(this, x + w / 2, y + h / 2, "empty", {
            fontSize: "10px",
            color: "#4c5a70",
          }).setOrigin(0.5),
        );
    ...
    this.slotLayer.add(
      crispText(this, 24, 96, `${this.slots.filter(Boolean).length}/${SQUAD_MAX} chosen`, {
        fontSize: "11px",
        color: "#cfe0f5",
      }),
    );
  ...
  private makeSlotTile(
    ...
    c.add(
      crispText(this, -w / 2 + 44, -10, def.name, {
        fontSize: "8px",
    ...
    c.on("pointerup", () => {
      if (!this.didDrag) this.select(def.id);
    });
    return c;
  }
  ...
  private drawGrid(save: ReturnType<SaveManager["getSave"]>): void {
    ...
    if (owned.length === 0) {
      this.dyn.add(
        crispText(this, 24, 162, "No characters here — summon some in the Summon Hall!", {
          fontSize: "12px",
          color: "#90a4bb",
        }),
      );
    ...
    const COLS = 7,
      ...
      BOTTOM = 470;
    owned.forEach((t, idx) => {
      const cx = X0 + (idx % COLS) * CW,
        cy = Y0 + Math.floor(idx / COLS) * CH;
      ...
      this.dyn.add(
        this.makeCharTile(
          ...
          CW - 8,
          CH - 8,
          getTowerStars(save, t.id),
          getTowerCopies(save, t.id),
          this.slots.includes(t.id),
        ),
      );
    });
  }
  ...
  private drawSkillPicker(save: ReturnType<SaveManager["getSave"]>): void {
    ...
    if (skills.length === 0) {
      this.dyn.add(
        crispText(this, 92, 482, "none yet (skills drop from battles)", {
          fontSize: "10px",
          color: "#90a4bb",
        }),
      );
    ...
    skills.slice(0, 6).forEach((entry, idx) => {
      ...
      const chipX = 92 + idx * 104,
        chipY = 480;
      const chip = crispText(this, chipX, chipY, `${def.name} L${entry.level}`, {
        fontSize: "10px",
        color: equipped ? "#fff" : RARITY_HEX[def.rarity],
        backgroundColor: equipped ? "#5a2a7a" : "#1a2230",
      })
        .setPadding(6, 3, 6, 3)
        .setInteractive({ useHandCursor: true });
      ...
      frame
        .lineStyle(1.5, DMG_INT[def.damageType] ?? 0x9fb0c4, 1)
        .strokeRoundedRect(chipX - 1, chipY - 1, chip.width + 2, chip.height + 2, 4);
      this.dyn.add(frame);
      chip.on("pointerup", () => {
        if (this.didDrag) return;
        if (equipped) this.mgr.unequipSkill(entry.skillId);
        else this.mgr.equipSkill(entry.skillId);
        this.redraw();
      });
      this.dyn.add(chip);
    });
  ...
  private makeCharTile(
    ...
    c.add(
      crispText(this, 0, h / 2 - 16, t.name, {
        fontSize: "8px",
    ...
    if (stars > 0)
      c.add(
        crispText(this, -w / 2 + 4, -h / 2 + 3, "★".repeat(stars), {
          fontSize: "9px",
          color: "#ffd24a",
        }),
      );
    ...
    c.setInteractive({ useHandCursor: true, draggable: true });
    c.on("pointerup", () => {
      if (!this.didDrag) this.select(t.id);
    });
    return c;
  }
  ...
  private drawPanel(save: ReturnType<SaveManager["getSave"]>): void {
    ...
    hg.fillStyle(heroSel ? 0x24344e : 0x16202e, 1).fillRoundedRect(
      PANEL_X + 8,
      PANEL_Y + 8,
      PANEL_W - 16,
      30,
    ...
    hg.lineStyle(2, heroSel ? 0x7ec8ff : 0x3a4a64, 1).strokeRoundedRect(
      PANEL_X + 8,
      PANEL_Y + 8,
      PANEL_W - 16,
      30,
    ...
    this.panel.add(hg);
    this.panel.add(
      crispText(this, PANEL_X + PANEL_W / 2, PANEL_Y + 14, "⚔  Your Hero", {
        fontSize: "12px",
        color: heroSel ? "#fff" : "#9fb0c4",
        fontStyle: "bold",
      }).setOrigin(0.5, 0),
    );
    const hz = this.add
      .zone(PANEL_X + 8, PANEL_Y + 8, PANEL_W - 16, 30)
      .setOrigin(0)
      .setInteractive({ useHandCursor: true });
    hz.on("pointerup", () => {
      if (!this.didDrag) this.select(HERO_SEL);
    });
    this.panel.add(hz);
