...
export class ExpeditionScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.picked = (save.meta.expedition.towerIds ?? [])
      .filter((id) => eligible.has(id))
      .slice(0, MAX_EXPEDITION_TOWERS);

    crispText(this, W / 2, 10, "🧭 Choose Expedition Heroes", {
      fontSize: "22px",
      color: "#ffd700",
      fontStyle: "bold",
    })
  ...
  private toggle(id: string): void {
    ...
    } else if (this.picked.length < MAX_EXPEDITION_TOWERS) {
      this.picked.push(id);
    } else {
      this.showToast(`Party is full (max ${MAX_EXPEDITION_TOWERS}) — tap a chosen hero to remove.`);
      return;
    }
    this.redraw();
  }
  ...
  private redraw(): void {
    ...
    this.layer.add(g);
    this.layer.add(
      crispText(this, 40, 50, `Party ${this.picked.length}/${MAX_EXPEDITION_TOWERS}`, {
        fontSize: "15px",
        color: "#ffe9b0",
        fontStyle: "bold",
      }),
    ...
    this.layer.add(
      crispText(
        ...
        40,
        68,
        `Gathers ${rate} 🪙/hr  ·  collectable after ${MIN_COLLECT_MIN}m, caps at 8h`,
        { fontSize: "12px", color: this.picked.length ? "#ffd56a" : "#9fb0c4" },
      ),
    );
    ...
    if (enabled) {
      const btn = crispText(
        ...
        W - 40,
        63,
        save.meta.expedition.startedAt > 0 ? "Re-dispatch" : "Dispatch",
        { fontSize: "15px", color: "#ffffff", backgroundColor: "#1f8f43", fontStyle: "bold" },
      )
        .setOrigin(1, 0.5)
      ...
      this.layer.add(
        crispText(this, W - 40, 63, "Pick at least 1 hero", {
          fontSize: "13px",
          color: "#6b7a8d",
        }).setOrigin(1, 0.5),
      );
  ...
  private drawGrid(save: ReturnType<SaveManager["getSave"]>): void {
    ...
    const owned = TOWERS.filter((t) => eligibleIds.has(t.id)).sort(
  ...
  private makeTile(
    ...
    g.lineStyle(
      picked ? 3 : 1.5,
      picked ? 0x52c878 : RARITY_INT[t.rarity],
      picked ? 1 : 0.85,
    ...
    if (picked)
      c.add(
        crispText(this, w / 2 - 6, -h / 2 + 3, `${order + 1}`, {
          fontSize: "12px",
          color: "#a5f0b8",
          fontStyle: "bold",
        }).setOrigin(1, 0),
