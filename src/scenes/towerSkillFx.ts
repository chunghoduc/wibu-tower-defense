...
type ShapeFn = (d: VfxDraw, at: V, p: Palette, radius: number, pw: VfxPower) => void;

// splash — staged concentric shock-rings punch outward; more rings at high rarity.
const nova: ShapeFn = (d, at, p, radius, pw) => {
  d.disc(at, 20, p.hot, 0.85, 2.0, 220);
  const rings = scaleCount(3, pw);
  for (let i = 0; i < rings; i++)
    d.after(i * 90, () => d.ring(at, radius * (0.5 + i * 0.24), i ? p.hot : p.core, 460, 4));
};

// chain — jagged links leap out in sequence to orbit points, each sparking.
const chain: ShapeFn = (d, at, p, radius, pw) => {
  const links = scaleCount(4, pw);
  for (let i = 0; i < links; i++) {
    const a = (Math.PI * 2 * i) / links + 0.4;
...
// barrage — stutter muzzle-strikes in a row, each spitting a tracer.
const barrage: ShapeFn = (d, at, p, radius, pw) => {
  const shots = scaleCount(4, pw);
  for (let i = 0; i < shots; i++)
    d.after(i * 50, () => {
      const off = { x: at.x + (i - (shots - 1) / 2) * 9, y: at.y };
...
export function renderTowerShape(
  ...
  palette: Palette,
  radius: number,
  pw: VfxPower = vfxPower(),
): void {
  SHAPES[shape](d, at, palette, radius, pw);
}
