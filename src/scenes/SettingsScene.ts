...
export class SettingsScene extends Phaser.Scene {
  ...
  private render(): void {
    ...
    if (!this.confirmingReset) {
      this.button(cx, y, "↺  Reset Game Progress", "#7a2e2e", () => { this.confirmingReset = true; this.render(); });
    } else {
  ...
  private changeVolume(delta: number): void {
    const v = Math.max(0, Math.min(1, Math.round((this.mgr.getSettings().volume + delta) * 100) / 100));
  ...
  private label(x: number, y: number, text: string, color = "#dfe7f2", size = 16): Phaser.GameObjects.Text {
    const t = crispText(this, x, y, text, { fontSize: `${size}px`, color }).setOrigin(0.5);
    t.setData("ctl", true);
    return t;
  }

  private toggle(x: number, y: number, label: string, on: boolean, cb: (on: boolean) => void): void {
    ...
    const z = this.add.zone(x + 90, y - h / 2, w, h).setOrigin(0).setInteractive({ useHandCursor: true });
    z.setData("ctl", true);
    z.on("pointerdown", () => cb(!on));
  }

  private stepBtn(x: number, y: number, glyph: string, cb: () => void): void {
    const t = crispText(this, x, y, glyph, { fontSize: "20px", color: "#fff", backgroundColor: "#2a3a56" })
      .setOrigin(0.5).setPadding(10, 4, 10, 4).setInteractive({ useHandCursor: true });
    t.setData("ctl", true);
    t.on("pointerdown", cb);
  }

  private button(x: number, y: number, label: string, bg: string, cb: () => void, width = 280): void {
    const t = crispText(this, x, y, label, { fontSize: "16px", color: "#fff", backgroundColor: bg, fixedWidth: width, align: "center" })
      .setOrigin(0.5).setPadding(0, 10, 0, 10).setInteractive({ useHandCursor: true });
    t.setData("ctl", true);
    t.on("pointerdown", cb);
  }
}
