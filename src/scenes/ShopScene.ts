...
export class ShopScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add
      .text(W / 2, 12, "🏪 Shop", { fontSize: "24px", color: "#ffd700", fontStyle: "bold" })
      .setOrigin(0.5, 0);
    this.add
      .text(20, 12, "← Back", { fontSize: "15px", color: "#90caf9" })
      .setInteractive({ useHandCursor: true })
      .on("pointerdown", () => fadeToScene(this, "MainMenuScene"));
    this.crystalText = this.add
      .text(W - 20, 14, "", { fontSize: "16px", color: "#bfe4ff" })
      .setOrigin(1, 0);

    this.tabBuy = this.tab(W / 2 - 96, "Buy", () => this.setMode("buy"));
    this.tabSell = this.tab(W / 2 + 6, "Recycle", () => this.setMode("recycle"));

    this.refreshBtn = crispText(this, W - 20, 46, this.refreshLabel(), {
      fontSize: "12px",
      color: "#fff",
    ...
    this.hoverLabel = this.add
      .text(W / 2, 478, "", { fontSize: "13px", color: "#e8eef6" })
      .setOrigin(0.5);
    ...
    this.input.on("wheel", (_p: Phaser.Input.Pointer, _o: unknown, _dx: number, dy: number) => {
      ...
      if (next !== this.sellOffset) {
        this.sellOffset = next;
        this.redraw();
      }
    });
  ...
  private tab(x: number, label: string, cb: () => void): Phaser.GameObjects.Text {
    const t = crispText(this, x, 44, label, {
      fontSize: "14px",
      color: "#fff",
  ...
  private setMode(m: "buy" | "recycle"): void {
    this.mode = m;
    this.redraw();
  }
  ...
  private redraw(): void {
    ...
    this.tabBuy
      .setBackgroundColor(this.mode === "buy" ? "#2a4a6a" : "#1a2a3a")
      .setAlpha(this.mode === "buy" ? 1 : 0.7);
    this.tabSell
      .setBackgroundColor(this.mode === "recycle" ? "#2a4a6a" : "#1a2a3a")
      .setAlpha(this.mode === "recycle" ? 1 : 0.7);
  ...
  private drawBuy(): void {
    ...
    if (stock.length === 0) {
      this.grid.add(
        crispText(this, this.scale.width / 2, 220, "Sold out! Refresh for new stock.", {
          fontSize: "14px",
          color: "#90a4bb",
        }).setOrigin(0.5),
      );
  ...
  private drawBuyCard(slot: ShopStockEntry, x: number, y: number, w: number, h: number): void {
    ...
    const afford = isDiamondSlot
      ? save.currency.diamonds >= slot.cost
      : save.currency.gold >= slot.cost;
    ...
    this.grid.add(
      crispText(
        ...
        x + w / 2,
        y + h - 50,
        isScroll ? "Rare Consumable" : `${def?.rarity} ${def?.slot}`,
        { fontSize: "10px", color: "#9fb0c4" },
      ).setOrigin(0.5),
    );
    this.grid.add(
      crispText(this, x + w / 2, y + h - 32, `${priceEmoji} ${slot.cost}`, {
        fontSize: "15px",
        color: afford ? "#ffe07a" : "#ff7a7a",
        fontStyle: "bold",
      }).setOrigin(0.5),
    );

    const z = this.add.zone(x, y, w, h).setOrigin(0).setInteractive({ useHandCursor: true });
    hoverGlowRect(this, z, this.grid, x, y, w, h, {
      radius: 8,
      color: isScroll ? SCROLL_GOLD : col,
    });
  ...
  private drawRecycle(
    ...
    if (sellable.length === 0) {
      const msg =
        this.sellCategory === "all" ? "No spare items to recycle." : "No items in this category.";
      this.grid.add(
        crispText(this, this.scale.width / 2, 220, msg, {
          fontSize: "14px",
          color: "#90a4bb",
        }).setOrigin(0.5),
      );
    ...
    drawScrollbar(this, this.grid, {
      x: X0 + COLS * (CW + GX) - GX + 2,
      y: Y0,
      h: visibleRows * rowH - GY,
      total: totalRows,
      visible: visibleRows,
      offset: this.sellOffset,
    });
  }

  private drawRecycleCard(
    ...
    if ((inst.enhanceLevel ?? 0) > 0)
      this.grid.add(
        crispText(this, x + w - 4, y + 3, `+${inst.enhanceLevel}`, {
          fontSize: "9px",
          color: "#ffe07a",
          fontStyle: "bold",
        }).setOrigin(1, 0),
    ...
    this.grid.add(
      crispText(this, x + w / 2, y + h - 16, `❖ ${chaos}`, {
        fontSize: "11px",
        color: "#ff9ec4",
        fontStyle: "bold",
      }).setOrigin(0.5),
    ...
    z.on("pointerover", () => {
      ...
      if (def)
        renderItemTooltip(this, this.tooltip, inst, def, x + w, y, this.mgr.getSave().hero.level);
    });
    z.on("pointerout", () => {
      this.hoverLabel.setText("");
      this.tooltip.setVisible(false);
    ...
    z.on("pointerup", () => {
      if (!this.sellDrag.didScroll()) this.openRecycle(inst, def?.name ?? "Item", def?.rarity);
    });
    this.grid.add(z);
  }
  ...
  private openRecycle(inst: ItemInstanceSave, name: string, rarity: Rarity | undefined): void {
    ...
    c.add(
      crispText(this, W / 2, by + 16, "Recycle", {
        fontSize: "16px",
        color: "#ffffff",
        fontStyle: "bold",
      }).setOrigin(0.5, 0),
    ...
    if (cost) {
      ...
      const reforge = crispText(
        ...
        `⟳ Reforge affixes\n🪙 ${cost.gold} + ❖ ${cost.chaos}`,
        {
          ...
          backgroundColor: have ? "#3a4a7a" : "#2a3142",
          fixedWidth: bw - 48,
          align: "center",
        },
      )
  ...
  private flash(msg: string, ok: boolean): void {
    ...
    this.time.delayedCall(1600, () => {
      if (this.feedback.text === msg) this.feedback.setText("");
    });
  }
}
