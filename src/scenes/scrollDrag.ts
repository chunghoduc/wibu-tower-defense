...
export function dragOffset(
  startOffset: number, startY: number, currentY: number, rowH: number, maxOffset: number,
): number {
...
export function attachDragScroll(scene: Phaser.Scene, cfg: DragScrollConfig): DragScrollHandle {
  let tracking = false, startY = 0, startOffset = 0, scrolled = false;
  ...
  scene.input.on("pointerdown", (p: Phaser.Input.Pointer) => {
    ...
    tracking = true; startY = p.y; startOffset = cfg.getOffset();
  });

  scene.input.on("pointermove", (p: Phaser.Input.Pointer) => {
    if (!tracking || !p.isDown) return;
    if (cfg.blocked && cfg.blocked()) { tracking = false; return; } // a tile-drag took over
    if (Math.abs(p.y - startY) > 4) scrolled = true;
    const next = dragOffset(startOffset, startY, p.y, cfg.rowH, cfg.maxOffset());
    if (next !== cfg.getOffset()) { cfg.setOffset(next); cfg.onChange(); }
  });

  scene.input.on("pointerup", () => { tracking = false; });
