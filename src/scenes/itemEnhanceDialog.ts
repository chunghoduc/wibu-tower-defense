...
export function renderEnhanceDialog(
  ...
  const render = () => {
    ...
    const add = (
      ...
      dialog.add(
        crispText(scene, dx + xx, dy + yy, txt, { fontSize: "13px", color: "#dfe8f3", ...style }),
      );
    ...
    rows.forEach((row, i) => {
      ...
      add(244, ry, row.after, {
        fontSize: "13px",
        color: maxed ? "#dfe8f3" : "#6ee06e",
        fontStyle: "bold",
      });
    });
    if (rows.length === 0)
    ...
    const btn = crispText(
      ...
      maxed ? "MAX" : canDo ? "⚒  Enhance" : "Need jewel",
      {
        fontSize: "15px",
        color: "#fff",
        backgroundColor: canDo ? "#1565c0" : "#444",
      },
    )
      .setOrigin(0.5, 0)
      .setPadding(16, 8, 16, 8)
      .setAlpha(canDo ? 1 : 0.6);
    if (canDo) {
    ...
    const close = crispText(scene, dx + W - 14, dy + 10, "✕", {
      fontSize: "16px",
      color: "#ef9a9a",
    })
      .setOrigin(1, 0)
