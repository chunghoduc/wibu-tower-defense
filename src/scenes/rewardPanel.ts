...
//
// The post-battle reward display: every drop from a cleared stage shown as an
// icon tile (gold, diamonds, gear, jewels, skills, characters, materials).
// Hovering a tile reveals its detail — equipment uses the full stat tooltip,
// everything else a titled info card. The icons carry the meaning; names live
// in the hover, per design. The pure tile mapping lives in data/rewardTiles.ts.
...
import { rewardTileSpecs, type RewardTileSpec } from "../data/rewardTiles.ts";
...
/**
 * Build the reward panel at (centerX, topY): a centred, wrapping grid of icon
 * tiles with hover tooltips. Returns the root container (already added to the
 * scene at depth 25). The caller owns its lifetime.
 */
export function showRewardPanel(
  scene: Phaser.Scene, result: DropResult, centerX: number, topY: number,
): Phaser.GameObjects.Container {
  const specs = rewardTileSpecs(result);
  const root = scene.add.container(0, 0).setDepth(25);
  const tooltip = scene.add.container(0, 0).setDepth(120).setVisible(false);

  // The scene already shows a big "VICTORY" banner above this; we add only a
  // small first-clear flourish (when relevant) so the two don't fight.
  let gridTop = topY;
  if (result.isFirstClear) {
    root.add(crispText(scene, centerX, topY, "★ First Clear bonus!", {
      fontSize: "13px", color: "#ffe07a", fontStyle: "bold", stroke: "#0a1420", strokeThickness: 4,
    }).setOrigin(0.5, 0));
    gridTop = topY + 20;
  }
