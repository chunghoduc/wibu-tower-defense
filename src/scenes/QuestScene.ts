...
const COLS = 2, CW = 452, CH = 96, X0 = 20, Y0 = 60, GAP_X = 16, GAP_Y = 10;
...
export class QuestScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    crispText(this, W / 2, 10, "📜 Daily Quests", { fontSize: "24px", color: "#ffd700", fontStyle: "bold" }).setOrigin(0.5, 0);
    ...
    this.toast = crispText(this, W / 2, 516, "", { fontSize: "13px", color: "#ffe1a8", backgroundColor: "#2a1f14" })
  ...
  private redraw(): void {
    ...
    this.layer.add(crispText(this, W / 2, 40,
      `${claimed}/${DAILY_QUESTS.length} claimed` + (claimable > 0 ? `  ·  ${claimable} reward${claimable > 1 ? "s" : ""} ready!` : "  ·  resets at midnight"),
      { fontSize: "12px", color: claimable > 0 ? "#ffd56a" : "#90a4bb" }).setOrigin(0.5, 0));
  ...
  private drawCard(def: QuestDef, x: number, y: number, save: ReturnType<SaveManager["getSave"]>): void {
    ...
    g.fillStyle(claimed ? 0x16221a : done ? 0x1f2a18 : 0x161d28, 1).fillRoundedRect(x, y, CW, CH, 10);
    g.lineStyle(2, claimable ? 0xffc94d : claimed ? 0x3a6b4a : 0x2c3a4f, 1).strokeRoundedRect(x, y, CW, CH, 10);
    this.layer.add(g);

    // Title + reward.
    this.layer.add(crispText(this, x + 14, y + 10, def.label, { fontSize: "16px", color: claimed ? "#8fc7a0" : "#ffe9b0", fontStyle: "bold" }));
    this.layer.add(crispText(this, x + CW - 14, y + 12, questRewardLabel(def.reward), { fontSize: "12px", color: "#ffe07a" }).setOrigin(1, 0));
    this.layer.add(crispText(this, x + 14, y + 32, def.description, { fontSize: "11px", color: "#aab8cc", wordWrap: { width: CW - 130 } }));

    // Progress bar.
    const bx = x + 14, by = y + CH - 22, bw = CW - 150, frac = Phaser.Math.Clamp(progress / def.target, 0, 1);
    const bar = this.add.graphics();
    bar.fillStyle(0x000000, 0.5).fillRoundedRect(bx, by, bw, 12, 4);
    bar.fillStyle(done ? 0x53c46a : 0x4a8cff, 1).fillRoundedRect(bx, by, Math.max(2, bw * frac), 12, 4);
    this.layer.add(bar);
    this.layer.add(crispText(this, bx + bw + 6, by - 1, `${Math.min(progress, def.target)}/${def.target}`, { fontSize: "11px", color: "#cdd6e6" }));

    // Claim button / status badge (bottom-right).
    const cbx = x + CW - 14, cby = y + CH - 24;
    if (claimed) {
      this.layer.add(crispText(this, cbx, cby, "✓ Claimed", { fontSize: "12px", color: "#9fe0b0", fontStyle: "bold" }).setOrigin(1, 0));
    } else if (claimable) {
      const btn = crispText(this, cbx, cby, "Claim", { fontSize: "13px", color: "#ffffff", backgroundColor: "#1f8f43", fontStyle: "bold" })
        .setOrigin(1, 0).setPadding(12, 5, 12, 5).setInteractive({ useHandCursor: true });
      ...
      this.layer.add(btn);
      this.tweens.add({ targets: btn, scale: 1.06, duration: 520, yoyo: true, repeat: -1, ease: "Sine.easeInOut" });
    } else {
      this.layer.add(crispText(this, cbx, cby, "In progress", { fontSize: "12px", color: "#6b7a8d" }).setOrigin(1, 0));
    }
  }
  ...
  private drawBonusBanner(save: ReturnType<SaveManager["getSave"]>): void {
    ...
    g.lineStyle(2, ready ? 0xffd24d : collected ? 0x3a6b4a : 0x33405a, 1).strokeRoundedRect(x, y, w, h, 10);
    this.layer.add(g);

    this.layer.add(crispText(this, x + 16, y + 8, "🏆 Complete all daily quests", { fontSize: "15px", color: "#ffe9b0", fontStyle: "bold" }));
    this.layer.add(crispText(this, x + 16, y + 29, `Reward: +${ALL_BONUS_DIAMONDS} 💎`, { fontSize: "12px", color: "#ffe07a" }));

    const cbx = x + w - 16, cby = y + h / 2;
    if (collected) {
      this.layer.add(crispText(this, cbx, cby, "✓ Collected", { fontSize: "13px", color: "#9fe0b0", fontStyle: "bold" }).setOrigin(1, 0.5));
    } else if (ready) {
      const btn = crispText(this, cbx, cby, "Claim Bonus", { fontSize: "14px", color: "#ffffff", backgroundColor: "#9a6a1f", fontStyle: "bold" })
      ...
      this.layer.add(crispText(this, cbx, cby, "Claim every quest first", { fontSize: "12px", color: "#6b7a8d" }).setOrigin(1, 0.5));
    }
  }
