...
export class ImpactFx {
  ...
  fire(at: Vec2): void {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const e = this.fac.circle(at.x, at.y, 1.5 + Math.random() * 1.5, i % 2 ? 0xffb14d : 0xff7a2a).setDepth(this.depth + 1).setBlendMode(ADD);
      this.scene.tweens.add({
        targets: e, x: at.x + Math.cos(a) * reach, y: at.y + Math.sin(a) * reach - 8, alpha: 0, scale: 0.2,
        duration: 320 + Math.random() * 160, ease: "Quad.easeOut", onComplete: () => e.destroy(),
      });
    }
  }
  ...
  ice(at: Vec2): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const shard = this.fac.star(at.x, at.y, 3, 1, 4, 0xcdeeff).setRotation(a).setDepth(this.depth + 1).setBlendMode(ADD);
      this.scene.tweens.add({
        targets: shard, x: at.x + Math.cos(a) * reach, y: at.y + Math.sin(a) * reach, alpha: 0, scale: 0.3, rotation: a + 1,
        duration: 280, ease: "Quint.easeOut", onComplete: () => shard.destroy(),
      });
    }
  }
  ...
  shock(at: Vec2): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const g = this.fac.graphics().setDepth(this.depth + 1).setBlendMode(ADD);
      ...
      g.strokePath();
      this.scene.tweens.add({ targets: g, alpha: 0, duration: 180, ease: "Quad.easeIn", onComplete: () => g.destroy() });
    }
  }
  ...
  arcane(at: Vec2): void {
    ...
    for (let i = 0; i < n; i++) {
      const a = (TAU * i) / n;
      const start = 18;
      const p = this.fac.circle(at.x + Math.cos(a) * start, at.y + Math.sin(a) * start, 2, 0xeec6ff).setDepth(this.depth + 1).setBlendMode(ADD);
      this.scene.tweens.add({
        targets: p, x: at.x, y: at.y, alpha: 0.4, scale: 0.5,
    ...
    this.scene.time.delayedCall(150, () => {
      ...
      const ring = this.fac.star(at.x, at.y, 6, 4, 8, 0xc77dde).setStrokeStyle(2, 0xeec6ff, 0.9).setDepth(this.depth + 1).setBlendMode(ADD);
      ring.setFillStyle(0xc77dde, 0);
      this.scene.tweens.add({ targets: ring, scale: 3.2, alpha: 0, rotation: 0.9, duration: 300, ease: "Cubic.easeOut", onComplete: () => ring.destroy() });
    });
  }
  ...
  poison(at: Vec2): void {
    ...
    this.scene.tweens.add({ targets: cloud, scale: 2, alpha: 0, duration: 620, ease: "Sine.easeOut", onComplete: () => cloud.destroy() });
    for (let i = 0; i < 6; i++) {
      const a = (TAU * i) / 6 + Math.random() * 0.6;
      const reach = 9 + Math.random() * 9;
      const drop = this.fac.circle(at.x, at.y, 1.5 + Math.random(), 0xb7e36a).setDepth(this.depth + 1);
      this.scene.tweens.add({
        targets: drop, x: at.x + Math.cos(a) * reach, y: at.y + Math.sin(a) * reach, alpha: 0, scale: 0.4,
        duration: 340, ease: "Quad.easeOut", onComplete: () => drop.destroy(),
      });
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      this.scene.tweens.add({ targets: b, y: at.y - 10 - Math.random() * 6, alpha: 0, duration: 500, delay: 80 + i * 60, ease: "Sine.easeOut", onComplete: () => b.destroy() });
    }
  }
  ...
  holy(at: Vec2): void {
    ...
    const pillar = this.fac.rectangle(at.x, at.y - 14, 7, 34, 0xffffff, 0.55).setDepth(this.depth + 1).setBlendMode(ADD);
    pillar.setScale(1, 0.2);
    this.scene.tweens.add({ targets: pillar, scaleY: 1.1, alpha: 0, duration: 300, ease: "Quad.easeOut", onComplete: () => pillar.destroy() });
    for (let i = 0; i < 5; i++) {
      const a = -Math.PI / 2 + (Math.random() - 0.5) * 1.2;
      const s = this.fac.circle(at.x, at.y, 1.5, 0xfff6cf).setDepth(this.depth + 1).setBlendMode(ADD);
      this.scene.tweens.add({ targets: s, x: at.x + Math.cos(a) * 12, y: at.y + Math.sin(a) * 14, alpha: 0, scale: 0.2, duration: 360, ease: "Quad.easeOut", onComplete: () => s.destroy() });
    }
  }
  ...
  cannon(at: Vec2, ang: number): void {
    ...
    for (let i = 0; i < 3; i++) {
      const px = at.x + (Math.random() - 0.5) * 8, py = at.y + (Math.random() - 0.5) * 8;
      const puff = this.fac.circle(px, py, 5, 0x6a6f7a, 0.45).setDepth(this.depth);
      this.scene.tweens.add({ targets: puff, scale: 2.4, alpha: 0, x: px + Math.cos(ang) * 6, y: py - 4, duration: 480, ease: "Sine.easeOut", onComplete: () => puff.destroy() });
    }
    // Debris — dark flecks flung roughly forward of the shot.
    for (let i = 0; i < 6; i++) {
      const a = ang + (Math.random() - 0.5) * 2.2;
      const reach = 12 + Math.random() * 10;
      const d = this.fac.rectangle(at.x, at.y, 2.5, 2.5, 0x3a3f48).setRotation(a).setDepth(this.depth + 1);
      this.scene.tweens.add({ targets: d, x: at.x + Math.cos(a) * reach, y: at.y + Math.sin(a) * reach + 4, alpha: 0, scale: 0.3, duration: 360, ease: "Quad.easeOut", onComplete: () => d.destroy() });
    }
  ...
  pierce(at: Vec2, ang: number): void {
    for (let i = 0; i < 4; i++) {
      ...
      this.scene.tweens.add({ targets: p, x: at.x + Math.cos(a) * reach, y: at.y + Math.sin(a) * reach, alpha: 0, scale: 0.3, duration: 240, ease: "Quad.easeOut", onComplete: () => p.destroy() });
    }
    const streak = this.fac.rectangle(at.x, at.y, 10, 2, 0xfff1c8, 0.8).setRotation(ang).setOrigin(0.5).setDepth(this.depth + 1).setBlendMode(ADD);
    this.scene.tweens.add({ targets: streak, scaleX: 0.2, alpha: 0, duration: 150, ease: "Quad.easeOut", onComplete: () => streak.destroy() });
  }
  ...
  bullet(at: Vec2, color: number): void {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const p = this.fac.rectangle(at.x, at.y, 3, 1.5, color).setRotation(a).setOrigin(0, 0.5).setDepth(this.depth + 1).setBlendMode(ADD);
      this.scene.tweens.add({ targets: p, x: at.x + Math.cos(a) * reach, y: at.y + Math.sin(a) * reach, alpha: 0, scaleX: 0.2, duration: 180, ease: "Quint.easeOut", onComplete: () => p.destroy() });
    }
  }
  ...
  private bloom(at: Vec2, color: number, r: number, dur: number): void {
    const c = this.fac.circle(at.x, at.y, r, color, 0.85).setDepth(this.depth + 1).setBlendMode(ADD);
    this.scene.tweens.add({ targets: c, scale: 2.4, alpha: 0, duration: dur, ease: "Quad.easeOut", onComplete: () => c.destroy() });
  }

  /** A bright impact flash. Additive. */
  private flash(at: Vec2, color: number, r: number, dur: number): void {
    const f = this.fac.circle(at.x, at.y, r, color, 0.9).setDepth(this.depth + 2).setBlendMode(ADD);
    this.scene.tweens.add({ targets: f, scale: 1.8, alpha: 0, duration: dur, ease: "Quad.easeOut", onComplete: () => f.destroy() });
  }
