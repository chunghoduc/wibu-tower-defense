...
interface Entry { t?: number; level?: string; tag?: string; msg?: string; data?: unknown; }
...
function fmt(e: Entry): string {
  ...
  if (e.data !== undefined) {
    let d: string;
    try { d = typeof e.data === "string" ? e.data : JSON.stringify(e.data); } catch { d = String(e.data); }
    line += ` ${d}`;
  }
...
async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (Array.isArray(entries) && entries.length) appendFileSync(LOG_FILE, entries.map(fmt).join(""));
...
export function fileLog(): Plugin {
  const mount = (middlewares: { use: (path: string, fn: (req: IncomingMessage, res: ServerResponse, next: () => void) => void) => void }) => {
  ...
  return {
    name: "wtd-file-log",
    configureServer(server) { initFile("dev"); mount(server.middlewares); },
    configurePreviewServer(server) { initFile("preview"); mount(server.middlewares); },
  };
}
