...
_hero_ is the one collecting the spoils. The request:
...
its _real_ inventory art flying in, consistent with the loot-icon fix already
shipped.
...
- No change to the HUD gold counter logic (gold is still credited the same; only
  the _coin visual_ destination changes from HUD corner to the hero).
