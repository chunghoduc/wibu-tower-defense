...
- **Towers can never block a corridor.** Free placement rejects any spot within
  `LANE_CLEARANCE` (30px) of the lane (`battle.ts:243`). So roads are _always_ walkable —
...
everywhere, to a central core") at the cost of _true_ free-roam pathing, which the game's own
...
`terrain: []` (the maze _is_ the terrain — no scattered blockers cluttering corridors).
