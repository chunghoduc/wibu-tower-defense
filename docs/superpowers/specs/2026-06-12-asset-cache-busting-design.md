...
**returning** player loads the _new_ manifest (fresh JS) but their browser keeps
the _old_ PNG forever (`immutable`). If the old cached PNG's frame layout differs
...
1. **Version-stamp generated-asset URLs.** A single `ASSET_VERSION` string is
   ...
   repairs caches in the wild _now_.
