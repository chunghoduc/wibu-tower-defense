...
function buildJobs() {
  ...
  jobs.push({
    ...
    id: "hero",
    anim: true,
    prompt: charSheetPrompt(`${HERO_BASE} ${HERO_WEAPON.sword}`),
    seed: seedOf("ahero"),
  });
  for (const [wt, d] of Object.entries(HERO_WEAPON))
    jobs.push({
      ...
      id: `hero__${wt}`,
      anim: true,
      prompt: charSheetPrompt(`${HERO_BASE} ${d}`),
      seed: seedOf("ahero"),
    });
  for (const [id, v] of Object.entries(ENEMY_VISUAL))
    jobs.push({
      ...
      id,
      anim: true,
      prompt: charSheetPrompt(v, { creature: true }),
      seed: seedOf("a" + id),
    });
  ...
  if (sample) {
    ...
    jobs = jobs.filter(
      (j) =>
        ...
        (j.kind === "hero" && j.id === "hero") ||
        (j.kind === "enemy" && j.id === "gargoyle") ||
        (j.kind === "boss" && j.id === "overlord") ||
        (j.kind === "item" && j.id === "arcane-staff"),
    );
  }
  ...
  for (const j of jobs) {
    ...
    if (j.anim) {
      ...
      try {
        execFileSync(
          "python3",
          [SLICE, raw, out, "--cell", j.boss ? "160" : "128", "--max-frames", "8"],
          { stdio: "inherit" },
        );
      } catch (e) {
      ...
      writeFileSync(raw, buf);
      try {
        execFileSync("python3", [CUTOUT, raw, out, "--size", "160", "--tol", "52", "--pad", "6"], {
          stdio: "ignore",
        });
      } catch (e) {
        console.log("   cut fail: " + e.message);
      }
    }
  }
  console.log("done");
