...
function turret(): CharacterDef {
  return {
    ...
    passives: ["p"],
    active: null,
    baseStats: makeStats({ atk: 100000, attackSpeed: 10, range: 9999, maxHp: 100 }),
    artRef: "placeholder",
  };
}
...
function world(e: EnemyDef, c: CharacterDef, waves: WaveDef[], castleHp: number, gold = 0) {
  return {
    ...
    catalog: { enemies: new Map([[e.id, e]]), characters: new Map([[c.id, c]]) },
  };
}
const inertHero = {
...
const oneWave = (n: number): WaveDef[] => [
  { spawns: [{ enemyId: "grunt", count: n, interval: 0.05, delay: 0 }] },
];
...
describe("F13 combo multiplier", () => {
  it("comboMult ramps from 1 toward COMBO_MAX_MULT and emits combo fx", () => {
    ...
    const maxComboMult = Math.max(
      ...fx
        .filter((e): e is Extract<FxEvent, { type: "combo" }> => e.type === "combo")
        .map((e) => e.mult),
    );
...
describe("F14 perfect wave", () => {
  it("emits a perfect event + bonus when no enemy leaks", () => {
    ...
    const fx: FxEvent[] = [];
    run(b, fx);
    const perfect = fx.find(
      (e): e is Extract<FxEvent, { type: "perfect" }> => e.type === "perfect",
    );
...
describe("F5 challenge modifiers in battle", () => {
  it("enemyHpMul makes enemies tankier; towerCostMul discounts placement", () => {
    ...
    const buffed = new BattleState(stage, catalog, {
      hero: inertHero,
      eliteChance: 0,
      challenge: { enemyHpMul: 2, towerCostMul: 0.5 },
    });
