...
import {
  WAVE_INTERVAL, SKIP_COIN_PER_SEC, INTER_WAVE_DELAY, AUTO_SKIP_COUNTDOWN,
...
const grunt = mkEnemy({ baseStats: makeStats({ maxHp: 100, moveSpeed: 50, atk: 0, attackSpeed: 0 }) });
const wave = (count: number) => ({ spawns: [{ enemyId: "grunt", count, interval: 0.3, delay: 0 }] });
const stage = () =>
  ({
    id: "test", name: "Test",
    path: [{ x: 0, y: 0 }, { x: 1e6, y: 0 }],
    airSpawns: [], castleHp: 50, startingGold: 100,
...
describe("early-clear auto-skip", () => {
  it("arms a 3s countdown and banks the time-saved bonus when the field is cleared early", () => {
    ...
    expect(b.gold).toBeGreaterThanOrEqual(goldBefore + Math.round((remaining - 6) * SKIP_COIN_PER_SEC));
