...
describe("stage numbering continuity", () => {
  it("numbers stages 1..20 with the right chapter prefixes", () => {
...
describe("difficulty-unlock bands for the expansion", () => {
  it("groups Chapter 2 and 3 into their own 5-stage bands", () => {
...
describe("depth-scaled gold reward", () => {
  ...
  it("pays strictly more for a deeper stage on the same difficulty", () => {
    ...
    const deep = processStageClear(createFreshSave(), "ch3-s20", "Normal", new Rng(7));
    expect(deep.goldAwarded).toBeGreaterThan(early.goldAwarded);
    // Stage 20 ≈ 2.14× the base; both first clears so the bonus cancels in ratio.
